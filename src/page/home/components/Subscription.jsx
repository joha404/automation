              ...
              >
                ...
                <div className="relative flex flex-col flex-1 p-6">
                  <div className="flex items-center justify-center gap-2 mb-3 flex-wrap">
                    ...
                    {plan.badge_text && (
                      <span
                        className="text-[11px] font-bold rounded-full py-2 px-3"
...
function QuarterlyFeaturedCard({ plan }) {
  return (
    ...
    >
      <div className="relative flex items-center justify-between h-full px-10 py-8 min-h-[269px]">
        {/* LEFT */}
        <div className="flex flex-col items-center justify-center">
          <div className="mb-3">
            <span
              className="font-extrabold text-[11px] tracking-widest uppercase px-4 py-1.5 rounded-md text-white"
              style={{ background: "#0A9087" }}
            >
              MOST POPULAR
            </span>
          ...
          <div className="flex items-center gap-2 mb-2">
            <span className="text-white font-bold text-[13px] tracking-widest uppercase">
              {plan.name}
            </span>
            {plan.badge_text && (
              <span
        ...
        <div className="flex flex-col gap-3 flex-1 mt-8 mb-8">
          {plan?.features?.map((f, i) => (
            <motion.div
              ...
              initial={{ opacity: 0, x: -8 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.3 + idx * 0.1 + i * 0.06, duration: 0.35 }}
            >
              <CheckIcon />
              <span className="text-white/80 text-[15px] font-medium">
                {f.name}
...
export default function Subscription({ data }) {
  ...
  const quarterlyPlans = plans.filter((p) => p.billing_period === "quarterly");
  const otherPlans = plans.filter((p) => p.billing_period !== "quarterly");

  const featuredQuarterly = quarterlyPlans.find((p) => p.is_most_popular);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {quarterlyPlans.length > 0 && (
            <div className="mb-10">
              ...
              >
                <span
                  className="text-[11px] font-extrabold tracking-widest uppercase px-3 py-1 rounded-md"
                  style={{
                    background: "rgba(10,144,135,0.18)",
                    color: "#0ee8d8",
                  }}
                >
                  QUARTERLY
