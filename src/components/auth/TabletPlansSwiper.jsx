...
import TabletPlanCard from "./TabletPlanCard";
import "swiper/css";
import "swiper/css/bundle";

export default function TabletPlansSwiper({
  plans,
  calculatePrice,
  calculateSavings,
  isMonthly,
  ...
  const handleContactUs = () => {
    window.location.href =
      "mailto:customersupport@hyperpicks.ai?subject=Hyper Picks Enterprise API Demo Request";
  };

  return (
    <div className="hidden md:block lg:hidden mb-6 w-full px-4">
      <Swiper
        slidesPerView={1.8}
        spaceBetween={20}
        ...
        className="tablet-plans-swiper !overflow-visible"
      >
        {plans.map((plan, index) => {
          // Check if this is the Enterprise API card
          if (plan.isEnterprise) {
            return (
              <SwiperSlide key={`enterprise-api-tablet-${index}`}>
                <div
                  className={`group relative rounded-2xl p-8 border transition-all duration-400 ease-in flex flex-col ${
                    isDark
                ...
                >
                  ...
                  <div className="text-center mb-6 relative z-10">
                    <p
                      className={`font-bold mb-3 uppercase tracking-widest text-sm ${
                  ...
                  <div className="flex-grow space-y-4 mb-6">
                    {plan.features.map((feature, featureIndex) => (
                      <div key={featureIndex} className="flex items-start">
                        <div
                          className={`flex-shrink-0 flex items-center justify-center mr-3 w-6 h-6 rounded-full transition-all duration-300 ${
                            isDark
                  ...
                  <div className="mt-auto pt-4">
                    <button
                      ...
                      className={`w-full px-6 py-3.5 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105 ${
                        isDark
          ...
          // Create a plan object that matches what TabletPlanCard expects
          const planCardData = {
            id: packageId,
            name: plan.region,
          ...
          return (
            <SwiperSlide key={`${plan.region}-tablet-${index}-${packageId}`}>
              <TabletPlanCard
                plan={planCardData}
