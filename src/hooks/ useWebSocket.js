...
export const useWebSocket = (token, user, scrollToBottom) => {
      ...
      switch (data.type) {
              ...
              const existingIndex = prev.findIndex(
                (msg) =>
                  ...
                  (msg.isOptimistic &&
                    ...
                    Math.abs(Date.now() - parseInt(msg.id.split("_")[1] || 0)) <
                      10000),
              );
        ...
        case "member_list": {
          if (data.members && Array.isArray(data.members)) {
            ...
            console.log("✅ Online members:", newOnlineCount);
            console.log("❌ Offline members:", newOfflineCount);
            ...
            if (!initialLoadCompleteRef.current) {
              if (
                ...
                console.log(
                  "🎯 Initial load complete with stable count:",
                  newOnlineCount,
                );
              ...
              if (shouldUpdate) {
                console.log("🔄 Updating online count:", newOnlineCount);
                setOnlineUsers(online);
                setOfflineUsers(offline);
                setOnlineCount(newOnlineCount);
                setOfflineCount(newOfflineCount);
  ...
  useEffect(() => {
    ...
    const wsUrl = import.meta.env.VITE_USS_URL_TEST + token;
